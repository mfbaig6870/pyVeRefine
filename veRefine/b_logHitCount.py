...
# Count the number of hits for each RPM and MAP bin
hit_count = df.groupby([map_bins, rpm_bins]).size()

# Reshape the resulting Series into a grid of RPM vs MAP with hit counts displayed in each cell
grid = hit_count.unstack().sort_index(axis=0, ascending=False)

# Save the resulting DataFrame as a CSV file
grid.to_csv('../outputs/logHitCount.csv')
