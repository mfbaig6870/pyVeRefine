...
# Group by RPM and MAP bins and calculate mean AFR value
df_grouped = df.groupby([map_bins, rpm_bins])['PCM1.P1.Final_AFR1'].mean().fillna(0)

# Round the mean AFR values to the nearest thousandth
df_grouped = df_grouped.round(3)

# Reshape the resulting Series into a grid of RPM vs MAP with mean AFR values displayed in each cell
grid = df_grouped.unstack().sort_index(axis=0, ascending=False)

# Save the resulting DataFrame as a CSV file
grid.to_csv('../outputs/avgAfrBins.csv')
